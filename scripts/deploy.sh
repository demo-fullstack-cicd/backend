...
aws s3 cp s3://$(grep S3_BUCKET_NAME /home/ec2-user/.env | cut -d '=' -f2)/.env /home/ec2-user/.env
...
# 로그에 찍어보기
echo "DOCKER_USERNAME: $DOCKER_USERNAME" | tee -a /var/log/codedeploy-output.log
echo "DOCKER_PASSWORD: $DOCKER_PASSWORD" | tee -a /var/log/codedeploy-output.log
